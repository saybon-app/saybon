...
app.get("/api/checkFormattingAccess", async(req,res)=>{ try{ var jobId = req.query.jobId; var passkey = req.query.passkey; if(!jobId || !passkey) return res.json({hasAccess:false}); var jobDoc = await db.collection("translationJobs").doc(jobId).get(); if(!jobDoc.exists) return res.json({hasAccess:false}); var job = jobDoc.data(); if(!job.formattingUnlocked || !job.selectedSubmissionId) return res.json({hasAccess:false}); var subDoc = await db.collection("jobSubmissions").doc(job.selectedSubmissionId).get(); if(!subDoc.exists) return res.json({hasAccess:false}); var sub = subDoc.data(); if(sub.translator !== passkey) return res.json({hasAccess:false}); res.json({ hasAccess:true, clientFile: job.clientFile, targetLanguage: job.targetLanguage, originalFileUrl: job.originalFileUrl, originalFileType: job.originalFileType, translation: sub.translation }); }catch(err){ console.error(err); res.status(500).json({hasAccess:false}); } });

app.post("/api/completeFormatting", express.json(), async(req,res)=>{ try{ var jobId = req.body.jobId; var finalDeliverableUrl = req.body.finalDeliverableUrl; if(!jobId || !finalDeliverableUrl) return res.status(400).json({error:"jobId and finalDeliverableUrl are required"}); await db.collection("translationJobs").doc(jobId).update({ formattingComplete: true, finalDeliverableUrl: finalDeliverableUrl, formattingCompletedAt: admin.firestore.FieldValue.serverTimestamp() }); res.json({success:true}); }catch(err){ console.error(err); res.status(500).json({error:"could not complete formatting"}); } });
